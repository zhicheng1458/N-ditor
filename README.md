...
- There are no grid snapping to entities (for now), this allows for finer placement of entity.

- Entity selection are done in a tray. Use scroll key to scroll through the tray to find the entity you would like to use.

- Entity rotation can now be additionally controlled by the key `Q`, `E`, `Z`, and `C`, indicating diagonal direction.

- Entity mode are selected via the numeric key `1`, `2`, `3`, `4`, following the format as given in the [N++ level data](https://i.imgur.com/qjm4AEc.png)
...
- There are no mode display for entities yet.

- There are no UI to select the level to import/export

- There are no internal format to save your work in its entirety (aka items outside of the boundary).
